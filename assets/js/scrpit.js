$(document).ready(function () {
    ...
    function getMovieProviders() {
        $.ajax({
            ...
            method: 'GET',
            headers: {
                'Authorization': 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIyZjc5MjRhMWU5MGU1MGMzNTVlZGYzNzk4ZTBiZjQwMCIsInN1YiI6IjY1NzI3NGRiMjExY2U1MDExYmFjY2FkZjYyMiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.Q4XaeeNH7quaqUxYCtRma0vhqh6js0QBiN4MQxfZY2s',
    ...
    function getMovieDetailsFromOMDB(imdbId) {
        ...
        $.ajax({
            ...
            success: function (omdbData) {
                ...
                const rottenTomatoesRating = omdbData.Ratings.find(rating => rating.Source === 'Rotten Tomatoes');
    
                // Now you can use these ratings to update your UI or perform other actions
                console.log('IMDb Rating:', imdbRating);
                console.log('Rotten Tomatoes Rating:', rottenTomatoesRating ? rottenTomatoesRating.Value : 'N/A');
    
                // Update UI with ratings
                $('#imdbRating').text(`IMDb Rating: ${imdbRating}`);
                $('#rottenTomatoesRating').text(`Rotten Tomatoes Rating: ${rottenTomatoesRating ? rottenTomatoesRating.Value : 'N/A'}`);
            },
            error: function (error) {
